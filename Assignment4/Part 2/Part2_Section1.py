...
# TODO: Replace <FILL IN> with appropriate code
trainingWithMyRatingsRDD = sc.union([trainingRDD, myRatingsRDD])
...
myUnratedMoviesRDD = (moviesRDD
                      .map(lambda x: (myUserID, x[0])).filter(lambda x: x not in myRatedMovies))
...
ratingsWithNamesRDD = (predictedWithCountsRDD
                       .join(moviesRDD).map(lambda x: (x[1][0][0], x[1][1], x[1][0][1]))).filter(lambda y: y[2] > 75)
